...
let
  ...
  geoip-updater = pkgs.writeScriptBin "geoip-updater" ''
    ...
    for dbName in "GeoLite2-City" "GeoLite2-Country" "GeoLite2-ASN"; do
      apiKey=$(cat ${config.sops.secrets.maxmind_api_key.path} | sed 's/\n//g')
      dbBaseUrl="https://download.maxmind.com/app/geoip_download?edition_id=$dbName&license_key=$apiKey&suffix=tar.gz";
