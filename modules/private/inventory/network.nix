...
let
  ...
  hosts = {
    ...
    pakhet = {
      ip = "10.255.101.200";
      ip6 = "${ulaPrefix}:101::200";
      mac = "58:9c:fc:0e:56:98";
      dnat = [
        { proto = "tcp"; port = 25; }
        { proto = "tcp"; port = 80; }
        { proto = "tcp"; port = 443; }
  ...
  reverseDns = lib.mapAttrsToList
    (name: host:
      ...
      in
      ''"${rev}.in-addr.arpa. IN PTR ${fqdn}."''
    )
    hosts;

  # Reverse zones needed for unbound
  reverseZones = lib.unique (lib.mapAttrsToList
    (_: host:
      let
        parts = lib.splitString "." host.ip;
        rev3 = lib.concatStringsSep "." (lib.reverseList (lib.take 3 parts));
      in
      ''"${rev3}.in-addr.arpa." static''
    )
    hosts);
  ...
  forwardDns6 = lib.optionals enableIPv6ULA (lib.flatten (lib.mapAttrsToList
    (name: host:
      let names = hostDnsNames name host;
  ...
  reverseDns6 = lib.optionals enableIPv6ULA (lib.mapAttrsToList
    (name: host:
      let
        dnsName = builtins.head (hostDnsNames name host);
        fqdn = qualify dnsName;
  ...
  expandIp6 = addr:
    ...
    in
    lib.concatStringsSep ":" allGroups;

  reverseZones6 = lib.optionals enableIPv6ULA (lib.unique (lib.mapAttrsToList
    (_: host:
      let
        expanded = expandIp6 host.ip6;
        nibbles = lib.stringToCharacters (lib.replaceStrings [ ":" ] [ "" ] expanded);
      ...
      in
      ''"${rev12}.ip6.arpa." static''
    )
    hostsWithIp6));

  # DHCPv6 reservations: hosts with mac and ip6
  dhcp6Reservations = lib.optionals enableIPv6ULA (lib.mapAttrsToList
    (name: host: {
      hostname = name;
      hw-address = host.mac;
  ...
  dhcpReservations = lib.mapAttrsToList
    (name: host: {
      hostname = name;
      hw-address = host.mac;
      ip-address = host.ip;
